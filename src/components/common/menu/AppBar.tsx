import { toTitle } from '@/utils/StringUtils';
import AppBar from '@mui/material/AppBar';
import Avatar from '@mui/material/Avatar';
import Box from '@mui/material/Box';
import Button from '@mui/material/Button';
import Container from '@mui/material/Container';
import IconButton from '@mui/material/IconButton';
import Menu from '@mui/material/Menu';
import MenuItem from '@mui/material/MenuItem';
import Toolbar from '@mui/material/Toolbar';
import Tooltip from '@mui/material/Tooltip';
import Typography from '@mui/material/Typography';

import { AccountCircleOutlined, AddLinkOutlined, AddTaskOutlined, ArrowDropDown, ArticleOutlined, AssessmentOutlined, AssignmentOutlined, AttachFileOutlined, AttachmentOutlined, BorderColorOutlined, BusinessOutlined, CancelPresentationOutlined, CastForEducationOutlined, CastOutlined, ComputerOutlined, DateRangeOutlined, DeleteForever, Diversity1Outlined, Diversity3Outlined, EngineeringOutlined, GradingOutlined, GridViewOutlined, Groups3Outlined, HelpOutline, HistoryEduOutlined, HistoryOutlined, InterestsOutlined, KeyOutlined, ListAltOutlined, LocalLibrary, LocalLibraryOutlined, LockOpenOutlined, LogoutOutlined, ManageAccountsOutlined, ManageSearchOutlined, MarkEmailReadOutlined, MeetingRoomOutlined, NavigateNextOutlined, NoteAddOutlined, NotificationsOutlined, PersonAddAlt1, PersonAddAlt1Outlined, PlaylistAdd, PlaylistAddCheckOutlined, PlaylistAddOutlined, PostAddOutlined, PsychologyOutlined, ReceiptLongOutlined, RestoreFromTrash, SettingsOutlined, SpellcheckOutlined, SupportAgentOutlined, WorkOutline } from '@mui/icons-material';
import * as React from 'react';
import { Auth } from '@/utils/AuthUtils';
import { menuOptions, settings } from '@/utils/MenuOptions';

function ResponsiveAppBar() {
  const [openedMenu, setOpenedMenu] = React.useState<string | null>(null);
  const [anchorElUser, setAnchorElUser] = React.useState<null | HTMLElement>(null);
  const [userPermissions, setUserPermissions] = React.useState<any>([]);

  const handleOpenUserMenu = (event: React.MouseEvent<HTMLElement>, key: string) => {
    setOpenedMenu(key);
    setAnchorElUser(event.currentTarget);
  };

  const handleCloseUserMenu = () => {
    setAnchorElUser(null);
    setOpenedMenu(null);
  };

  React.useEffect(() => {
    setUserPermissions(Auth.getPermissions());
  }, [])

  return (
    <AppBar position="static" style={{ color: "#fff" }}>
      <Container maxWidth="xl">
        <Toolbar disableGutters>
          <Avatar src="../logo.png" />
          <Typography
            variant="h6"
            noWrap
            component="a"
            href="/"
            sx={{
              mr: 2,
              fontFamily: 'monospace',
              fontWeight: 700,
              letterSpacing: '.3rem',
              color: '#fff',
              textDecoration: 'none',
              marginLeft: '15px'
            }}
          >
            LUNA COLLEGES
          </Typography>
          <Box sx={{ flexGrow: 1, display: { xs: 'flex', md: 'flex' } }}>
            {menuOptions.map((option: any) => {
              const permissionKeys = option.options.map((o: any) => o.permissionKey);
              const permitted = permissionKeys.some((p: any) => (userPermissions?.includes(p) || p === '*'))
                return (
                  permitted ? <>
                    <Button
                      key={option.title}
                      onClick={(e) => option.options?.length > 0 ? handleOpenUserMenu(e, option.title) : window.location.href = `/${option.title}`}
                      style={{ marginLeft: "12px", color: "#fff" }}
                      startIcon={<option.icon />}
                      endIcon={option.options?.length > 0 && <ArrowDropDown />}
                    >
                      {toTitle(option.title)}
                    </Button>
                    <Menu
                      sx={{ mt: '45px' }}
                      id={`${option.title}-menu`}
                      anchorEl={anchorElUser}
                      anchorOrigin={{
                        vertical: 'top',
                        horizontal: 'right',
                      }}
                      keepMounted
                      transformOrigin={{
                        vertical: 'top',
                        horizontal: 'right',
                      }}
                      open={openedMenu === option.title}
                      onClose={handleCloseUserMenu}
                    >
                      {option.options.map((opt: any) => {
                          return (
                            userPermissions?.filter((p: any) => (p.includes(opt?.permissionKey)) || opt?.permissionKey === '*')[0] ? 
                            <MenuItem 
                                href={opt.path ? `/${opt.path}`: `/${opt.title}`}
                                component="a" 
                                key={opt.title}
                                onClick={() => window.location.href = opt.path ? `/${opt.path}`: `/${opt.title}`}>
                              <opt.icon />
                              <Typography style={{ marginLeft: "12px", marginRight: "12px" }} textAlign="center">{toTitle(opt.title)}</Typography>
                            </MenuItem> : null
                          );
                      })}
                    </Menu>
                  </> : null
                )
            }
            )}
          </Box>
          <Box sx={{ flexGrow: 0 }}>
            <Tooltip title="My Account">
              <IconButton onClick={(e) => handleOpenUserMenu(e, 'my-account')} sx={{ p: 0 }}>
                <Avatar alt="Remy Sharp" src="data:image/jpeg;base64," />
              </IconButton>
            </Tooltip>
            <Menu
              sx={{ mt: '45px' }}
              id="menu-appbar"
              anchorEl={anchorElUser}
              anchorOrigin={{
                vertical: 'top',
                horizontal: 'right',
              }}
              keepMounted
              transformOrigin={{
                vertical: 'top',
                horizontal: 'right',
              }}
              open={openedMenu === 'my-account'}
              onClose={handleCloseUserMenu}
            >
              {settings.map((setting: any) => (
                <MenuItem 
                  href={`/${setting.title}`}
                  component="a" 
                  key={setting.title} 
                  onClick={() => {
                    handleCloseUserMenu();
                    window.location.href = `/${setting.title}`;
                  }}
                >
                  <setting.icon />
                  <Typography textAlign="center" style={{ marginLeft: "12px" }}>{toTitle(setting.title)}</Typography>
                </MenuItem>
              ))}
            </Menu>
          </Box>
        </Toolbar>
      </Container>
    </AppBar>
  );
}
export default ResponsiveAppBar;